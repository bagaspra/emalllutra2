...
class Tripay_payment extends CI_Model{
    ...
    function getmetode($get="semua"){
        $arr = array(
            ...
            "bsiva"     => array("kode"=>"BSIVA","nama"=>"BSI Virtual Account","logo"=>"08.webp","biaya"=>4000),
        ...
        if(empty($err)){
            ...
            if($res["success"] == true AND count($res["data"]) > 0){
                for($i=0; $i<count($res["data"]); $i++){
                    $logo = (isset($logos[$res["data"][$i]["code"]])) ? base_url("asset/images/payment/".$logos[$res["data"][$i]["code"]]) : base_url("cdn/uploads/no-image.png");
            ...
            $hasil = array(
                "kode"  => "",
                "nama"  => "",
                "logo"  => base_url("asset/images/no-image.jpg"),
                "biaya" => 0,
                "biaya_merchant" => 0,
                "biaya_total" => 0,
                "active"=> false
    ...
    function createPayment($trx,$metode,$jumlah,$pembeli,$produk=null){
        ...
        $pembeli["nama"] = (isset($pembeli["nama"]) AND $pembeli["nama"] != "") ? $pembeli["nama"] : "Nama Orang";
        $pembeli["email"] = (isset($pembeli["email"]) AND $pembeli["email"] != "") ? $pembeli["email"] : $set->email;
        $pembeli["nohp"] = (isset($pembeli["nohp"]) AND $pembeli["nohp"] != "") ? $pembeli["nohp"] : $set->wasap;
        ...
        $data = [
            ...
            'signature'         => hash_hmac('sha256', $set->merchant_code.$trx.$jumlah, $set->private_key)
        ];
        ...
        if(!empty($err)){
            ...
            if($response["success"] != 1){
                ...
                $cek = $this->db->query("SELECT * FROM rb_tripay where kode_transaksi='$trx'");
                if ($cek->num_rows()>=1){
                    $this->db->where("kode_transaksi",$trx);
                    $this->db->update("rb_tripay",$data);
                }else{
                    $this->db->insert("rb_tripay",$data);
                }

                $datas = [
                    "catatan"=>$response["reference"],
                    "penampung"=>$metode,
                    "status_trx"=>'tripay'
                ];
                $this->db->where("kode_transaksi",$trx);
                $this->db->update("rb_penjualan_otomatis",$datas);
