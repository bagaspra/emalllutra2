...
class Main extends CI_Controller {
	...
	public function index(){
		...
		$this->template->load(template().'/template',template().'/'.$layout,$data);
	}
	...
	public function layout(){
		$layout = array("content", "content1", "content3");
		if (in_array($this->uri->segment(3), $layout)){
			$this->session->set_userdata(array('layout'=>cetak($this->uri->segment(3))));
			redirect('main');
		}else{
			redirect('main');
		}
