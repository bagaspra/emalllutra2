      ...
      <div class="row">
        <div class="col-xl-3 col-lg-3 col-md-12 col-sm-12 col-12 ">
          <?php
            ...
            <a href='".base_url()."u/".user_reseller(reseller($this->session->id_konsumen))."' class='ps-btn gray-btn btn-block custom-btn'><center>Kunjungi Toko</center></a>
        ...
        <div class="col-xl-9 col-lg-9 col-md-12 col-sm-12 col-12 ">
            ...
            $this->session->unset_userdata('message');
              ...
              <label class='col-sm-3 col-form-label' style='margin-bottom:1px'>URL Lapak</b></label>
                <div class='col-sm-9'>
              ...
              <div class='form-group row' style='margin-bottom:5px'>
              <label class='col-sm-3 col-form-label' style='margin-bottom:1px'>Kordinat di Peta</b></label>
                <div class='col-sm-9'>";
                if ($rows['kordinat']!=''){
                  echo "<a target='_BLANK' style='color:blue; text-decoration:underline' href='https://www.google.co.id/search?q=$rows[kordinat]'>$rows[kordinat]</a><br>";
                }else{
                  echo "-<br>";
                }
                  echo "<small style='color:red'><i><b>PENTING</b> - Kordinat Untuk Keperluan Hitung Jarak atau Ongkir Kurir Lokal.</i></small>
                </div>
              </div>
              ...
              if ($rows['referral']!='' AND $rows['referral']!='0'){
                ...
                <div class='col-sm-9'>
                  <i style='color:blue'>$ref[nama_lengkap]</i>
                </div>
                </div>";
              }
              ...
              <label class='col-sm-3 col-form-label' style='margin-bottom:1px'>Status Toko</b></label>
                <div class='col-sm-9'>";
                  $cek_paket = $this->db->query("SELECT * FROM rb_reseller_paket a JOIN rb_paket b ON a.id_paket=b.id_paket where a.id_reseller='".reseller($this->session->id_konsumen)."' AND a.paket='on'");
                  if ($cek_paket->num_rows()>=1){
                    foreach ($cek_paket->result_array() as $rowp) {
                      ...
                      }elseif ($rowp['status']=='N' AND $rowp['paket']!='on'){
