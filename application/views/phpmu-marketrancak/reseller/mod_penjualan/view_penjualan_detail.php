    ...
    <div class="container">
      <div class="ps-section__content">
        <?php 
          ...
          }elseif($rows['kode_kurir']!='0' AND $rows['kode_kurir']!='-'){
            $kolom = 6;
        ...
        <div class="row">
          <?php echo "<div class='col-xl-$kolom col-lg-$kolom col-md-$kolom col-sm-12 col-12'>"; ?>
            <div class='table-responsive'>
                <table class="table table-sm iconset">
                  <tbody>
                    ...
                    <?php 
                      if ($rows['keterangan']!=''){
                        ...
                        echo "<tr><td><strong>$judul</strong></td> <td><b>".$exal[5]."</b> ($exal[6])</td></tr>";
                      }else{
                        echo "<tr><td><strong>$judul</strong></td> <td><b>$rows[nama_lengkap]</b> ($rows[no_hp])<br>";
                      }
                    ?>
                    <tr><th scope='row'><b><?= $judl2; ?></b></th>               <td><?php echo alamat($rows['kode_transaksi']); ?></td></tr>
                    <?php 
                  ...
                  <?php 
                    ...
                    echo "<tr class='warning'>
                          ...
                          echo "<tr class='success'>
                            ...
                            <td><b>Rp ".rupiah(($total['total']+$detail['ongkir'])-($rows['fee']/100*$total['total'])-$kupon['diskon']-$fee_produk-$ck)."</b></td>
                          </tr>";
                  ...
                  <?php 
                    if ($cekk['jenis_produk']=='Digital'){
                      if ($rows['proses']==1){
                      echo "<form action='".base_url().$this->uri->segment(1)."/detail_penjualan/".$this->uri->segment(3)."' method='POST'>
                              <textarea class='textarea' name='no_resi' placeholder='Tulis Catatan Order,..'>$rows[no_resi]</textarea>
                              <button class='btn btn-primary btn-sm pull-right' style='padding: 4px 15px;' type='submit' name='submit'><span class='fa fa-check'></span> Tandai Sebagai Terkirim</button>
                            </form><br>";
                      }elseif ($rows['proses']==3 OR $rows['proses']==4){
                        echo "<p style='border: 1px solid green; padding: 10px; border-left: 5px solid green;'><b>Pesan dari Anda (Seller/Penjual)</b> : <br>$rows[no_resi]</p><br>";
                      }
                    }
                  ?>
              ...
              <?php 
                if($rows['kode_kurir']!='0' AND $rows['kode_kurir']!='' AND $rows['kode_kurir']!='-'){ // Jika Transaksi Bukan COD 
              ?>
