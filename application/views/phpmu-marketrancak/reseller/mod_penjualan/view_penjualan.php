      ...
      <div class="ps-section__content">
        ...
        <div class='table-responsive'>
                    ...
                    <ul class="nav nav-tabs" id="myTab" role="tablist">
                      ...
                      <li class="nav-item"><a class="nav-link <?php echo ($_GET['page'] == 'batal' ? 'active' : ''); ?>" href='<?= base_url(); ?>members/penjualan?page=batal'>Batal / Ditolak <span class="badge badge-secondary"><?php echo total_penjualan('x',reseller($this->session->id_konsumen)); ?></span></a></li>
                    </ul><br>
                      ...
                      <?php 
                        ...
                        echo "<tr><td>$no</td>
                                  ...
                                  if ($row['group_order']!=''){
                                    echo "<td><center><a class='btn btn-info btn-xs grouporder' href='#' data-id='$row[group_order]' style='font-size:13px; padding:2px 10px'><span class='fa fa-users'></span> Detail</a></center></td>";
                                  }else{
                                    echo "<td><center>";
                    ...
                    <table id="example11" class="table table-striped table-sm iconset">
                      ...
                      <?php 
                        ...
                        echo "<tr><td>$no</td>
                                  ...
                                  <td style='color:red;'>Rp ".rupiah($total['total']+$row['ongkir']-$kupon['diskon']-$fee_produk)."</td>";
                                    if ($row['group_order']!=''){
                                      echo "<td><center><a class='btn btn-success btn-xs grouporder' href='#' data-id='$row[group_order]' style='font-size:13px; padding:2px 10px'><span class='fa fa-users'></span> Detail</a>
                                      <a class='btn btn-danger btn-xs' title='Tolak Penjualan' href='".base_url().$this->uri->segment(1)."/tolak_penjualan/$row[id_penjualan]' onclick=\"return confirm('Apa anda yakin ingin TOLAK Pesanan ini?')\" style='font-size:13px; padding:2px 10px'><span class='fa fa-remove'></span> Tolak</a></center></td>";
                                    }else{
                                      echo "<td><center>
                                      <a class='btn btn-info btn-xs' target='_BLANK' title='Print Data' href='".base_url().$this->uri->segment(1)."/detail_penjualan/$row[id_penjualan]?print' style='font-size:13px; padding:2px 10px'><span class='fa fa-print'></span></a>
                    ...
                    <?php if ($_GET['page']=='batal'){ ?>
                    <table id="example14" class="table table-striped table-sm iconset">
                        <thead>
                          <tr>
                            <th style='width:40px'>No</th>
                            <th>Kode Transaksi</th>
                      ...
                      <?php 
                        $no = 1;
                        foreach ($batal->result_array() as $row){
                        if ($row['proses']=='0'){ $proses = '<i class="text-danger">Pending</i>'; $status = 'Proses'; $icon = 'star-empty'; $ubah = 1; }elseif($row['proses']=='1'){ $proses = '<i class="text-success">Proses</i>'; $status = 'Pending'; $icon = 'star text-yellow'; $ubah = 0; }elseif($row['proses']=='3'){ 
                            $proses = '<i class="text-success">Dikirim</i>'; $status = 'Dikirim'; $icon = 'star text-yellow'; $ubah = 0; 
                        }else{ $proses = '<i class="text-info">Konfirmasi</i>'; $status = 'Proses'; $icon = 'star'; $ubah = 1; }
                        $total = $this->db->query("SELECT sum((a.harga_jual-a.diskon)*a.jumlah) as total, sum(a.fee_produk_end*a.jumlah) as fee_produk FROM `rb_penjualan_detail` a where a.id_penjualan='$row[id_penjualan]'")->row_array();
                        ...
                        echo "<tr><td>$no</td>
                                  ...
                                  <td style='color:red;'>Rp ".rupiah($total['total']+$row['ongkir']-$kupon['diskon']-$fee_produk)."</td>";
                                    if ($row['group_order']!=''){
                                      ...
                                      echo "<td><center>
                                        <a class='btn btn-success btn-xs' title='Detail Data' href='".base_url().$this->uri->segment(1)."/detail_penjualan/$row[id_penjualan]' style='font-size:13px; padding:2px 10px'><span class='fa fa-search'></span> Detail</a></center></td>";
                                    }
                              echo "</tr>";
                          $no++;
                        }
